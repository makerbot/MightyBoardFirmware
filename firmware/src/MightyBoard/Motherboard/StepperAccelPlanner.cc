...
#define isqrt1(x) ((int32_t)sqrt((float)(x)))
	...
	if ((fres != 0.0) && ((fabsf(fres - FPTOF(result))/fres) > 0.01))
	{
	     ...
	     snprintf(buf, sizeof(buf),
		      ...
		      FPTOF(result), fres);
	     if (sblock)
		  strlcat(sblock->message, buf, sizeof(sblock->message));
	     else
		  printf("%s", buf);
	}
	return result;
  ...
  int32_t decelerate_after = accelerate_steps + plateau_steps;
	...
	if ((decelerate_after+1) < (int32_t)block->step_event_count) {
	     ...
	     if ( extruder_advance_k2 != 0 ) {
	        ...
	        // On the Replicator with 1/16th stepping we use >> 5
	        // Acceleration limit to prevent overflow is 0xFFFFF / axis-steps-per-mm
	        advance_pressure_relax = FPTOI(FPDIV(FPMULT3(extruder_advance_k2, ITOFP((int32_t)block->acceleration_st >> (1+MICROSTEPPING)), KCONSTANT_100), ITOFP((int32_t)decelerate_steps)));
	     }
...
void plan_buffer_line(FPTYPE feed_rate, const uint32_t &dda_rate, const uint8_t &extruder, bool use_accel)
{
...
//#endif
  ...
  if ( feed_rate != 0 ) { //DEBUG_PIN6.setValue(true);
  ...
  }else{//DEBUG_PIN7.setValue(true);
  }
  //DEBUG_PIN6.setValue(false);
  //DEBUG_PIN7.setValue(false);
